...
/* ---------------------------------------------------------------------
   Upload rekap BBM dari Excel — sheet dengan baris header berisi kolom
   "Tanggal", "Jenis" (Masuk/Keluar), "Alat"/"Sumber", dan "Liter" (nama
   kolom boleh beda kapitalisasi/tambahan kata, dicari otomatis). Mengganti
   data BBM proyek ini untuk rentang tanggal yang ada di file (supaya
   upload ulang bulan yang sama tidak dobel data) — sama seperti upload
   Kurva-S di atas.
--------------------------------------------------------------------- */
function parseBbmRekapSheet(workbook) {
  const preferredName = workbook.SheetNames.find((n) => /bbm|rekap|fuel/i.test(n));
  ...
  if (headerIdx === -1) {
    throw new Error('Format file tidak dikenali. File harus punya baris header dengan kolom "Tanggal", "Jenis" (isi Masuk/Keluar), "Alat" (atau "Sumber"), dan "Liter".');
  }
  const records = [];
  const skipped = [];
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    const equipment = String(row[colEquip] || "").trim();
  ...
  if (!records.length) throw new Error("Tidak ada baris data BBM yang valid ditemukan di file.");
  const dates = records.map((r) => r.date).sort();
  return { records, skipped, periodStart: dates[0], periodEnd: dates[dates.length - 1] };
}
...
$("#bbm-upload-input").addEventListener("change", (e) => {
  ...
  reader.onload = async (ev) => {
    try {
      const data = new Uint8Array(ev.target.result);
      const workbook = XLSX.read(data, { type: "array", cellDates: true });
      const { records, skipped, periodStart, periodEnd } = parseBbmRekapSheet(workbook);
      const projFilter = $("#bbmProjectFilter").value;
      const projectId = projFilter !== "all" ? projFilter : state.projectId;
      let msg = `Ditemukan ${records.length} baris data BBM (periode ${fmtDateLong(periodStart)} s.d ${fmtDateLong(periodEnd)}). Data BBM proyek ini di periode tersebut akan DIGANTI dengan isi file ini. Lanjutkan?`;
